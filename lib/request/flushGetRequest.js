 ...
 * @private
 */
module.exports = function flushGetRequest(
    request,
    pathSetArrayBatch,
    callback
) {
    ...
    var getRequest;
    try {
        console.error("xxxx flush " + request._attemptCount + " " + h(requestPaths));
    ...
    var disposable = getRequest.subscribe(
        ...
        function(err) {
            console.error("xxxx flush failed " + request._attemptCount + " " + h(requestPaths) + " " + JSON.stringify(requestPaths));
